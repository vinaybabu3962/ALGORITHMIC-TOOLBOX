...
<center>
<span style="background-image:linear-gradient(to right, #6db9ef, #7ce08a);color:black;" class="badge badge-pill badge-success"><h3>DIJKSTRAS ALGORITHM</h3></span>
<% String [] out1=(String[])request.getAttribute("out");
   int[] distance=(int[])request.getAttribute("distance");
   int start=(int)request.getAttribute("start"); %>
   <h3>Start Node: <%=start %> </h3>
<% for(i=0;i<out1.length;i++) {%>
	<% if(i==start ) {%>
		Distance from <%=start %> to node <%=i %> is : <%=0 %>, the path is <span style="font-weight:bold;"> <%=start %> to <%=start %></span>
		
	<%}else{ %>
		Distance from <%=start %> to node <%=(i) %> is : <%= distance[i]%> , the path is <span style="font-weight:bold;"> <%=out1[i] %></span>
	<%} %>
	<br>
<%} %>
</center>
